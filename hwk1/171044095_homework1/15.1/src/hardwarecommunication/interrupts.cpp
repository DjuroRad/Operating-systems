...
bool funTerminatedHandler(uint8_t interrupt){
    return interrupt == 0x0d;
}
...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interrupt, uint32_t esp)
{
    ...
    {
        if( !funTerminatedHandler(interrupt) ){
            ...
            if( !taskManager->getNumTasks() == 0 )
                esp = (uint32_t)taskManager->CurrentTaskFunctionTerminated((CPUState*)esp);//updates the task manager
            // esp = (uint32_t)taskManager->Schedule((CPUState*)esp);//now can perform the scheduling
            // return esp;
        }
        
        //returns 0x0D from the system when a function terminates.
        //This can be handled as a software interrupt
        //Though unnecessary since it will have desired behaviour even without it.
        

        //remove the current thread here means
